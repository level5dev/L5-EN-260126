...
export default class ProductDetails extends ProductDetailsBase {
  constructor($scope, context, productAttributesData = {}) {
    ...
    this.imageGallery = new ImageGallery(
      ...
      context,
    ); // custom gallery requires "context"
    ...
    getProductList("product", currentProductID)
      ...
      .catch(async (err) => {
        try {
          ...
          window.upsellProductList = csvProducts.filter(
            (prod) => prod.product_id != currentProductID,
          );
        } catch (err) {
          throw err;
        }
    ...
    this.freqBought = new FreqBoughtTogether({
      cartItems: this.context.cartItems,
    });
    this.addToCartUpsell = new AddToCartUpsell(
      this.$scope,
  ...
  addProductToCart(event, form) {
    ...
    upsellSuiteCPU.saveUpsellData(
      ...
      Array.from($(".productView-info-value.cpu-product")).map(
        (el) => el.innerText,
      ),
    );
  ...
  updateCartContent(modal, cartItemId, onComplete) {
    this.getCartContent(cartItemId, (err, response) => {
      ...
      utils.api.cart.getCartQuantity({}, (error, response) => {
        ...
        //$(".navUser-item--cart .countPill.cart-quantity").text(`(${quantity})`);
        $(".navUser-item--cart .countPill.cart-quantity").addClass(
          "countPill--positive",
        );
        //$("#totalincart").text(quantity);
      });
