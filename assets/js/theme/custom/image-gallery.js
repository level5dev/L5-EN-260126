...
export default class ImageGallery {
  ...
  setMainImage(imgObj) {
    ...
    this.setActiveThumb();
    this.swapMainImage();
    this.updateFirstThumbnailIfVariant(imgObj);
  }

  updateFirstThumbnailIfVariant(imgObj) {
    // Only update if this is a variant change (no thumbnail was clicked)
    if (!imgObj.$selectedThumb && this.$selectableImages.length > 0) {
      const $firstThumb = this.$selectableImages.first();
      //console.log('thumb ', this.currentImage)
      // Update thumbnail image
      //$firstThumb.find('img').attr('src', imgObj.mainImageUrl);

      // Update data attributes for zoom/click
      $firstThumb.attr({
        'data-image-gallery-new-image-url': imgObj.mainImageUrl,
        'data-image-gallery-zoom-image-url': imgObj.zoomImageUrl,
        'data-image-gallery-new-image-srcset': imgObj.mainImageUrl
      });

      $firstThumb.find('img').attr({
        src: this.currentImage.mainImageUrl,
        srcset: this.currentImage.mainImageSrcset,
        alt: this.currentImage.mainImageAlt,
        title: this.currentImage.mainImageAlt,
        'data-srcset': this.currentImage.mainImageUrl,
      });
  ...
  setAlternateImage(imgObj) {
    ...
    this.variantZoomImage = imgObj.zoomImageUrl;
    this.setMainImage(imgObj);
  }

  restoreFirstThumbnail() {
    if (this.savedImage.firstThumbData && this.$selectableImages.length > 0) {
      const $firstThumb = this.$selectableImages.first();
      const thumbData = this.savedImage.firstThumbData;

      // Restore all thumbnail image attributes
      $firstThumb.find('img').attr({
        src: thumbData.src,
        srcset: thumbData.srcset,
        alt: thumbData.alt,
        title: thumbData.title,
        'data-srcset': thumbData.dataSrcset,
      });

      // Restore data attributes
      $firstThumb.attr({
        'data-image-gallery-zoom-image-url': thumbData.zoomUrl,
        'data-image-gallery-new-image-url': thumbData.newUrl,
      });

      if (thumbData.newSrcset) {
        $firstThumb.attr('data-image-gallery-new-image-srcset', thumbData.newSrcset);
      }

      // Refresh Slick if exists
      const $slickContainer = $firstThumb.closest('.slick-slider');
  ...
  openPhotoSwipe(itemIndex) {
    //console.log("openPhotoSwipe", itemIndex);
    const pswpElement = document.querySelectorAll(".pswp")[0];
    let items = this.photoswipeItems.map(item => ({ ...item }));
    if (this.variantZoomImage && itemIndex === 0) {
      const img = new Image();
      img.src = this.variantZoomImage;

      // Wait for image to load to get proper dimensions
      img.onload = () => {
        items[0] = {
          src: this.variantZoomImage,
          w: img.naturalWidth,
          h: img.naturalHeight,
        };
      };

      // Also set it immediately with fallback dimensions
      items[0] = {
        src: this.variantZoomImage,
        w: img.width || 1500,
        h: img.height || 1500,
    ...
    /*const options = {
      ...
      closeOnScroll: false,
      closeOnVerticalDrag: false,
      history: false, // Disable browser history navigation
      shareEl: false, // Disable sharing options
      fullscreenEl: false, // Disable fullscreen button
      zoomEl: false, // Disable zoom button
      closeElClasses: ["item", "caption", "zoom-wrap", "ui", "top-bar"], // Add other classes that should close the gallery when clicked
    };*/
  ...
  handleClickImage(e) {
    ...
    const newIndex =
      isImage && this.videoFirstPos && hasVideos
  ...
  buildPhotoSwipeCollection() {
    ...
    const emptyArrayCount =
      imageCount + videoCount === 0 ? 1 : imageCount + videoCount;
    ...
    if (imageCount) {
      this.$selectableImages.each((index, el) => {
        const newIndex =
          videoCount && this.videoFirstPos ? index + videoCount : index;
    ...
    if (videoCount && !this.videoFirstPos) {
      this.$selectableVideos.each((index, el) => {
        const newIndex = imageCount ? index + imageCount : index;
        const videoIframe = $(el).attr("data-pswp-content");
        const videoImageSrc = $(el).attr("data-video-gallery-zoom-image-url");
  ...
  bindEvents() {
    ...
    this.$mainImage
      .find(".image-main")
      .on("click", this.handleClickImage.bind(this)); // makes clicking off in mobile work to hide
