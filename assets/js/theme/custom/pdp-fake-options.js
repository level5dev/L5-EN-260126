...
const getProductData = () => {
  return $("[data-option-product-ids]")
    .toArray()
    .map((item) => [
      `${item.id}`,
      $(item).data("option-product-ids").split(","),
    ]);
};

const getProductIds = (data) => {
  return data
    .reduce((acc, opt) => [...acc, ...opt[1]], [])
    .map((id) => parseInt(id));
};
...
export default function (context) {
  ...
  graphQLFetch(storefrontToken, body, (res) => {
    ...
    products.map((product) => {
      ...
      const cfLabel = product?.customFields.find(
        (cf) => cf.name === "__product-label",
      );
