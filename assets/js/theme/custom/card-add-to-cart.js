...
export default class CardAddToCart {
  constructor(context) {
    if (
      !context.itsConfig.card_atc_button ||
      context.itsConfig.card_atc_button_pos !== "bottom"
    )
      return;

    this.hasQtyInput = context.itsConfig.card_atc_input;
    this.defaultQty =
      typeof context.itsConfig.card_atc_input_default === "string"
        ? 0
        : context.itsConfig.card_atc_input_default;

    $("body").on(
      "facetedSearchRefresh productViewModeChanged",
  ...
  updateCard($target, type = null) {
    const $scope = $target.hasClass("js-card-atc")
      ? $target
      : $target.parents(".js-card-atc");
    switch (type) {
      case "loading":
  ...
  addItemToCart(url, $target) {
    ...
    const $cardAtcModal = $(".modal-body.card-atc-modal");
    ...
    $.post(url, (reponse) => {
      ...
      utils.api.cart.getCartQuantity({}, (error, response) => {
        ...
        const $cardAddedHeading = $(".modal-header-title");
        ...
        if (this.previewModal) {
          ...
          quantity > 1
            ? $cardAddedHeading.text(
                `${quantity} tools are in your cart. What's next?`,
              )
            : $cardAddedHeading.text("1 tool added in your cart. What's next?");
  ...
  wireAddToCartButton($cards) {
    $(".js-card-atc__button", $cards).on("click", (event) => {
      ...
      const $target = $(event.currentTarget);
      const $scope = $target.parents(".js-card-atc");
      const qty = this.hasQtyInput
        ? parseInt($(".js-card-atc__input--total", $scope).val(), 10)
        : 1;
